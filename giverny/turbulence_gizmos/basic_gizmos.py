...
"""
def check_cube_title(cube_title):
    # check that cube_title is a valid cube title.
    valid_cube_titles = ['isotropic4096', 'isotropic8192']
    
    if cube_title not in valid_cube_titles:
        raise Exception(f"'{cube_title}' (case-sensitive) is not a valid cube title: {valid_cube_titles}")
...
def check_timepoint(timepoint, cube_title):
    ...
    if timepoint not in valid_timepoints[cube_title]:
        raise Exception(f'{timepoint} is not a valid timepoint: must be in the inclusive range of ' +
                        f'[{valid_timepoints[cube_title][0]}, {valid_timepoints[cube_title][-1]}]')
...
mapping gizmos.
"""
def get_dataset_resolution(cube_title):
...
def get_filename_prefix(cube_title):
