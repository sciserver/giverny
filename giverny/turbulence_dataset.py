...
class turb_dataset():
    ...
    def __init__(self, dataset_title = '', output_path = '', auth_token = '', cube_dimensions = 3, rewrite_dataset_metadata = False, rewrite_interpolation_metadata = False):
        ...
        if self.dataset_title in giverny_datasets:
            ...
            if turb_dataset.cluster is None or turb_dataset.client is None:
    ...
    def initialize_dask(cls):
        ...
        with warnings.catch_warnings(record = True) as caught_warnings:
            ...
            cls.cluster = LocalCluster(n_workers = get_constants()['dask_maximum_processes'], processes = True, memory_limit = '4GB', silence_logs = logging.ERROR)
            cls.client = Client(cls.cluster)
        
        # iterate over any caught warnings.
        for caught_warning in caught_warnings:
            if caught_warning.category.__name__ == 'UserWarning':
                # warn the user if they have multiple notebook kernels running this code as that would mean several dask localclusters are spun up.
                print('warning: a dask cluster is already running. please shut down all python kernels before proceeding...')
            else:
                # display the warning messages if they are unexpected.
    ...
    def init_interpolation_cube_size_lookup_table(self, sint = 'none', sint_specified = 'none', read_metadata = False, rewrite_metadata = False):
        ...
        if read_metadata:
            ...
            # use sint_specified when one of the 'linear*' step-down methods is being used.
            sint_cube_size = sint_specified if 'linear' in sint else sint
            ...
            # assume the linear interpolation is being applied along the z-axis (i.e. 'sabl2048*' datasets).
            if sint == 'linear':
            ...
            elif sint in ['linear_g', 'linear_l', 'linear_h']:
                self.cube_min_index = np.array([self.cube_min_index, self.cube_min_index, 0])
    ...
    def read_database_files_in_parallel_with_dask(self, user_single_db_boxes):
        ...
        for disk_index, database_file_disk in enumerate(user_single_db_boxes):
            ...
            # read in the voxel data from all of the database files on this disk.
            result_output_data.append(turb_dataset.client.submit(self.get_iso_points, user_single_db_boxes[database_file_disk],
                                                                 self.getcutout_vars, self.open_file_vars,
    ...
    def read_visitors_in_parallel_variable(self, db_visitor_map, visitor_output_data):
        ...
        if len(db_visitor_map) != 0:
            ...
            for db_visitor_map_chunk, worker in zip(db_visitor_map_split, workers):
                ...
                temp_visitor_output_data.append(turb_dataset.client.submit(self.get_iso_points_variable_visitor, db_visitor_map_chunk,
    ...
    def read_database_files_in_parallel_with_dask_variable(self, db_native_map, db_visitor_map):
        ...
        if len(db_native_map) != 0:
            ...
            for disk_index, database_file_disk in enumerate(db_native_map):
                ...
                result_output_data.append(turb_dataset.client.submit(self.get_iso_points_variable, db_native_map[database_file_disk],
                                                                     self.open_file_vars, self.interpolate_vars,
                                                                     workers = worker, pure = False))
        ...
        if len(db_visitor_map) != 0:
            ...
            for db_visitor_map_chunk, worker in zip(db_visitor_map_split, workers):
                # submit the data for parallel processing.
                result_output_data.append(turb_dataset.client.submit(self.get_iso_points_variable_visitor, db_visitor_map_chunk,
                                                                     self.getdata_vars, self.open_file_vars, self.interpolate_vars,
                                                                     workers = worker, pure = False))
