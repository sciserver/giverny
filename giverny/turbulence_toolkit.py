...
def getCutout(cube, var_original, timepoint_original, axes_ranges_original, strides,
    ...
    var, var_offsets, axes_ranges, timepoint = \
        getCutout_housekeeping_procedures(query_type, dataset_title, axes_ranges_original, strides, var_original, timepoint_original, c)
    ...
    if dataset_title not in giverny_datasets:
        if auth_token == c['pyJHTDB_testing_token'] and num_datapoints > 4096:
            raise Exception(f'too many points requested for the testing auth token: {num_datapoints} > 4096')
    ...
    if dataset_title in giverny_datasets:
        ...
        result = getCutout_process_data(cube, axes_ranges, var, timepoint,
                                        axes_ranges_original, strides, var_original, var_offsets, timepoint_original)
    else:
        """
...
def getCutout_housekeeping_procedures(query_type, dataset_title, axes_ranges_original, strides, var_original, timepoint_original, c):
    """
    ...
    if dataset_title in giverny_datasets:
        ...
        if dataset_title not in ['sabl2048low', 'sabl2048high']:
            ...
            result = getData_process_data(cube, points, var, timepoint, temporal_method, spatial_method,
                                          var_original, var_offsets, timepoint_original, spatial_method_specified, option)
        else:
                ...
                for spatial_method_tmp in points_map:
                    ...
                    if var_original == 'velocity':
                        ...
                        result_tmp = getData_process_data(cube, points_tmp, var, timepoint_tmp, temporal_method, spatial_method_tmp,
                                                          var_original, var_offsets_tmp, timepoint_original, spatial_method_specified_tmp, option)
                        ...
                        # get the results.
                        result_tmp = getData_process_data(cube, points_tmp, var, timepoint_tmp, temporal_method, spatial_method_tmp,
                                                          var_original, var_offsets, timepoint_original, spatial_method_specified_tmp, option)
